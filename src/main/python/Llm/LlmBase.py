...
class LlmBase:
    ...
    def get_process_from_image_prompt(self):
        return "Extract the store address, store name as store_name, total cost as total_cost, date of purchase as date_of_purchase in date format and for all purchased items the quantity, name and price in the purchased_items list from the following hungarian or english receipt image as processed_receipt. Also return the extracted text as receipt_text. "+self.process_receipt_constraints() 
    
    def get_ocr_image_prompt(self):
        return "Please extract the text from the following hungarian or english receipt image as receipt_text. "
    
    def get_process_from_composite_prompt(self,separator,receipt_texts):
        return "You will reive a receipt extracted by multiple Ocr model separated by "+ separator +" characters. You have to combine these to recreate the original receipt, return it as receipt_text. Extract the store address, store name as store_name, total cost as total_cost, date of purchase as date_of_purchase in date format and for all purchased items the quantity, name and price in the purchased_items list from the following hungarian or english receipt as processed_receipt. "+self.process_receipt_constraints()+" The texts are:\n"+receipt_texts
